...
<div class="flex items-center justify-between mb-8">
    <div class="flex items-center gap-3">
        ...
        <h1 class="text-2xl font-semibold text-gray-800 tracking-tight">Monitoring Absensi</h1>
    </div>
    
    {{-- EXPORT --}}
    <form method="GET" action="{{ route('admin.absensi.export') }}">
...
<x-card>
    ...
    <div class="mb-8 p-5 bg-gray-50/80 rounded-2xl border border-gray-100">
        <form method="GET" action="{{ route('admin.absensi') }}" class="flex flex-wrap items-end gap-4">
        ...
        <table class="w-full">
            ...
            <tbody class="divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50/80 transition-colors">
                    <td class="px-5 py-4">
                        <div class="flex items-center gap-3">
                            <div class="w-8 h-8 bg-[#0B2D45]/10 rounded-full flex items-center justify-center">
                                <span class="text-xs font-medium text-[#155E76]">{{ substr($absen->user->name, 0, 1) }}</span>
                            </div>
                            <span class="text-sm font-medium text-gray-800">{{ $absen->user->name }}</span>
                        </div>
                    </td>
                    ...
                    <td class="px-5 py-4">
                        <span class="text-sm text-gray-800">{{ $absen->kegiatan }}</span>
                    </td>
